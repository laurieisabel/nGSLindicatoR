# Fonction créée 16 sept. 2021 par Jordan Ouellette-Plante modifée par jmc et part LI
...
#' @param prop Dataframe avec la proportion de chaque strate dans chaque région écosystémique.
...
get_pop_indices_RECO <- function(ans, set, catch, stratum, strates_sel = NULL, prop,
  ...
  if(compa.year.meth == 3){
    ...
    toto$keep <- ifelse(toto$annee == 1990 & toto$annee_trait %in% trait.comp & toto$nav == 31, 0, 
                        ifelse(toto$annee %in% 2004:2005 & toto$annee_trait %in% trait.comp & toto$nav == 34, 0,
  ...
  # Calcul par régions écosystémiques
  pop_ind <- data.frame()
  PID <- c(1,2,3)
  for (i in PID){
    pid <- prop %>% filter(PID == i)
    set_adj <- left_join(set, pid, by = c("no_str"))
    set_adj <- set_adj %>% filter(!is.na(prop))
    set_adj <- set_adj %>% mutate(surfkm91 = surfkm91*prop)
    new_strata <- set_adj %>% select(no_str, surfkm91) %>% distinct()
  ...
  a_t0 <- set_adj %>% 
    ...
    ungroup %>% 
    filter(n >= 1) %>% # ne conserve que les cellules année-strate où j'ai >= 1 traits
    group_by(annee) %>% 
  ...
  tmp <- setcatch %>%
    ...
    ungroup %>% 
    filter(n_traits >= 1) %>% # ne conserve que les cellues année-strate où j'ai >= 1 traits
    group_by(annee, no_str) %>% 
    summarise(n = n(),
              mean_n = mean(nb_capt_cor),
              mean_p = mean(pds_capt_cor),
              var_n = var(nb_capt_cor),
              var_p = var(pds_capt_cor),
    ...
    ungroup
  strata <- unique(new_strata$no_str)
  tmp <- tmp %>% filter(no_str %in% strata)
  tmp <- tmp %>% select(-surfkm91)
  tmp <- left_join(tmp, new_strata, by = "no_str")
  
  # Calcul de la moyenne et de la variance pour la zone d'étude (RECO), selon un plan d'éch. aléatoire stratifié
  tmp <- left_join(a_t0, tmp, by = "annee") %>% 
    ...
    select(annee, surf_ech, nb_mean, nb_icinf, nb_icsup, pds_mean, pds_icinf, pds_icsup)
  ...
  pue <- pue %>%
    ...
    mutate_at(.vars = c("pds_mean", "pds_icinf", "pds_icsup"),
              .funs = function(x) round(x, 7))
  
  pue$PID <- i
  pop_ind <- bind_rows(pop_ind, pue)
  }
  return(pop_ind)
}
